...
| 関数 | 説明 | 引数 |
|------|------|------|
| `受入確認(タイプ)` | Accept ヘッダーにタイプが含まれるか（q値対応） | MIMEタイプ |
| `フォーマット応答(マッピング)` | Accept に応じた分岐応答（q値優先度対応） | [["text/html", html関数], ["application/json", json関数], ...] |

### 新規 API（v5.1）

| 関数 | 説明 | 引数 |
|------|------|------|
| `IP一覧取得()` | X-Forwarded-For のプロキシチェーンを配列で取得（Trust Proxy 必須） | なし |
| `Vary設定(フィールド)` | Vary レスポンスヘッダーを追加（重複除外） | ヘッダーフィールド名 |
| `リンク設定(リンク配列)` | RFC 5988 Link ヘッダーを生成 | [["rel名", "URL"], ...] |
| `タイプ設定(タイプ)` | Content-Type を簡易指定 ("json"→"application/json; charset=utf-8" 等) | タイプ文字列 |
| `ステータス送信(コード)` | ステータスコードとそのテキストをボディとして送信 | ステータスコード |
| `フレッシュ確認()` | キャッシュの新鮮さを判定（ETag / If-Modified-Since） | なし |
| `陳腐確認()` | キャッシュが古いか判定（フレッシュ確認の反転） | なし |
...
| 関数 | 説明 | 引数 |
|------|------|------|
| `リクエスト取得(名前)` | リクエストヘッダーを名前で取得（req.get） | ヘッダー名 |
| `リクエスト種別(タイプ)` | Content-Type 判定（req.is）、省略形対応："json"→"application/json" | タイプ文字列 |
| `XHR確認()` | XMLHttpRequest かどうか判定（X-Requested-With ヘッダー） | なし |
| `サブドメイン取得()` | Host ヘッダーからサブドメインを配列で取得 | なし |
| `元URL取得()` | パス + クエリ文字列の結合URL（req.originalUrl） | なし |

#### レスポンスヘルパー

| 関数 | 説明 | 引数 |
|------|------|------|
| `レスポンス取得(名前)` | レスポンスヘッダーを名前で取得（res.get） | ヘッダー名 |
| `レスポンス追記(名前, 値)` | ヘッダーを追記/マージ（res.append） | ヘッダー名, 値 |
| `送信済確認()` | レスポンスが送信済みか判定（res.headersSent） | なし |

#### Cookie・セッション

| 関数 | 説明 | 引数 |
|------|------|------|
| `Cookie詳細設定(名前, 値, オプション)` | 構造化オプション付き Cookie 設定（path / domain / maxAge / httpOnly / secure / sameSite） | 名前, 値, オプション辞書 |
| `セッション再生成()` | セッション ID を再生成（セッション固定攻撃対策） | なし |

#### ローカル変数（res.locals）

| 関数 | 説明 | 引数 |
|------|------|------|
| `ローカル設定(キー, 値)` | レスポンスローカル変数を設定 | キー, 値 |
| `ローカル取得(キー)` | レスポンスローカル変数を取得 | キー |

#### 開発者ツール

| 関数 | 説明 | 引数 |
|------|------|------|
| `ログ形式(形式)` | ログ出力形式の設定："default" / "combined" / "dev" / "short" / "tiny" | 形式名 |
| `応答時間()` | X-Response-Time ヘッダーを自動付与 | なし |
| `リクエストID()` | X-Request-Id（UUID v4）を自動付与 | なし |
| `サーバー情報()` | サーバー情報を辞書で取得（バージョン/ポート/ルート数/スレッド等10項目） | なし |
...
**🔴 バグ修正:**
- HEAD リクエストが GET ルートに自動マッチするよう修正（find_route HEAD→GET フォールバック）
- HEAD レスポンスでボディを送信しないよう修正（ヘッダーのみ返却）
- If-Modified-Since の日付比較を文字列比較から RFC 7231/850/asctime 形式の正規パースに変更
- 405 レスポンスに Allow ヘッダーを正しく付与（RFC 7231 準拠）

**🟠 HTTP プロトコル改善:**
- `Expect: 100-continue` ヘッダー対応（100 Continue レスポンス自動送信）
- 静的ファイルの HEAD リクエスト対応（Content-Length 付きヘッダーのみ返却）
- グレースフルシャットダウン（SIGINT/SIGTERM 時に最大5秒間アクティブスレッド完了待ち）
- TCP_NODELAY 設定（Nagle アルゴリズム無効化でレスポンス遅延低減）

**🟡 新規 API（16関数）:**

*リクエストヘルパー:*
- `リクエスト取得(名前)` — ヘッダー取得（req.get）
- `リクエスト種別(タイプ)` — Content-Type 判定（req.is、省略形対応）
- `XHR確認()` — XMLHttpRequest 判定
- `サブドメイン取得()` — Host からサブドメイン配列取得
- `元URL取得()` — パス + クエリ文字列取得（req.originalUrl）

*レスポンスヘルパー:*
- `レスポンス取得(名前)` — レスポンスヘッダー取得（res.get）
- `レスポンス追記(名前, 値)` — ヘッダー追記/マージ（res.append）
- `送信済確認()` — ヘッダー送信済み判定（res.headersSent）

*Cookie・セッション:*
- `Cookie詳細設定(名前, 値, オプション辞書)` — 構造化オプション付き Cookie 設定
- `セッション再生成()` — セッション ID 再生成（セッション固定攻撃対策）

*ローカル変数:*
- `ローカル設定(キー, 値)` / `ローカル取得(キー)` — res.locals 相当のリクエストスコープ変数

*開発者ツール:*
- `ログ形式(形式)` — ログ形式設定（default/combined/dev/short/tiny）
- `応答時間()` — X-Response-Time ヘッダー自動付与（CLOCK_MONOTONIC 使用）
- `リクエストID()` — X-Request-Id（UUID v4）自動付与
- `サーバー情報()` — 10項目のサーバー情報辞書（v2: 辞書型に刷新）

**🔒 セキュリティ強化:**
- セッション ID 生成に暗号学的乱数使用（macOS: arc4random / Linux: /dev/urandom）
- セキュリティミドルウェアに CSP / HSTS / Permissions-Policy 追加
- セッション Cookie に SameSite=Lax + 条件付き Secure 追加
- セッション再生成によるセッション固定攻撃対策

### v5.1.0 — バグ修正・スレッド安全性・新API

**🔴 重大バグ修正:**
- Chunked Transfer 時の `callback_mutex` アンロック漏れを修正
- `value_to_json` のバッファオーバーフロー防止ガード追加
- ファイルアップロード時のパストラバーサル脆弱性を修正

**🟠 改善:**
- 静的リダイレクトに Location ヘッダーを正しく付与
- `keep_alive_timeout` 設定値が recv タイムアウトに反映されるよう修正
- セッション管理にスレッドセーフ mutex 追加
- レートリミッターにスレッドセーフ mutex 追加
- SSE mutex の二重初期化を除去
- `malloc` NULL チェック追加
- Accept ヘッダーの q= 品質値を正しくパース・ソート

**🟡 新規 API（7関数）:**
- `IP一覧取得()` — X-Forwarded-For プロキシチェーン取得
- `Vary設定(フィールド)` — Vary ヘッダー管理
- `リンク設定(リンク配列)` — RFC 5988 Link ヘッダー生成
- `タイプ設定(タイプ)` — 簡易 Content-Type 設定
- `ステータス送信(コード)` — ステータスコード＋テキスト送信
- `フレッシュ確認()` — キャッシュ新鮮さ判定
- `陳腐確認()` — キャッシュ陳腐化判定
