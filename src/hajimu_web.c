...
typedef struct {
    ...
    int    body_length;
    int    sent;
    int    chunked_started;   /* v5.4: チャンク転送状態 (per-response) */
    /* v5.2: res.locals — ミドルウェア→ハンドラ間のデータ共有 */
    KVPair locals[HW_MAX_QUERY_PARAMS];
    int    local_count;
} HttpResponse;
...
typedef struct {
    ...
    char       redirect_location[HW_MAX_PATH]; /* v5.1: 静的リダイレクトのLocation */
} Route;
...
static int g_ctx_key_initialized = 0;

static RequestContext *get_ctx(void) {
    return (RequestContext *)pthread_getspecific(g_ctx_key);
}
static HttpResponse *ctx_resp(void) {
    RequestContext *c = get_ctx(); return c ? c->resp : NULL;
}
static HttpRequest *ctx_req(void) {
    RequestContext *c = get_ctx(); return c ? c->req : NULL;
}
static socket_t ctx_fd(void) {
    RequestContext *c = get_ctx(); return c ? c->fd : INVALID_SOCK;
}
static char *ctx_session_id(void) {
    RequestContext *c = get_ctx(); return c ? c->session_id : NULL;
}
...
/* v5.2: グローバル変数 (前方宣言) */
static char g_log_format[64] = "default";
...
static void generate_session_id(char *id, int size) {
    ...
    for (int i = 0; i < size - 1; i++)
        id[i] = charset[arc4random_uniform((uint32_t)(sizeof(charset) - 1))];
#else
    /* Linux: /dev/urandom から読み取り */
    FILE *urand = fopen("/dev/urandom", "rb");
    if (urand) {
        unsigned char rbytes[128];
        int need = size - 1 < 128 ? size - 1 : 128;
        if (fread(rbytes, 1, need, urand) == (size_t)need) {
            for (int i = 0; i < need; i++)
                id[i] = charset[rbytes[i] % (sizeof(charset) - 1)];
        }
        fclose(urand);
    } else {
        /* フォールバック */
...
static Session *find_session(const char *id) {
    ...
    pthread_mutex_lock(&g_server.session_mutex);
    for (int i = 0; i < g_server.session_count; i++) {
        if (g_server.sessions[i].active &&
            ...
            pthread_mutex_unlock(&g_server.session_mutex);
...
static Session *create_session(void) {
    ...
    /* v5.4: 容量満杯時は最古セッションを強制期限切れ */
    if (!s) {
        int oldest = -1;
        time_t oldest_time = now;
        for (int i = 0; i < g_server.session_count; i++) {
            if (g_server.sessions[i].active &&
                g_server.sessions[i].last_access <= oldest_time) {
                oldest_time = g_server.sessions[i].last_access;
                oldest = i;
            }
        }
        if (oldest >= 0) {
            g_server.sessions[oldest].active = 0;
            s = &g_server.sessions[oldest];
        }
    }
    if (!s) {
        pthread_mutex_unlock(&g_server.session_mutex);
...
/* v5.4: スレッドセーフセッション操作 (TOCTOU防止) */
static int session_get_var_safe(const char *sid, const char *key,
                                char *out, int out_size) {
    if (!sid || !sid[0] || !key) return 0;
    pthread_mutex_lock(&g_server.session_mutex);
    for (int i = 0; i < g_server.session_count; i++) {
        if (g_server.sessions[i].active &&
            strcmp(g_server.sessions[i].id, sid) == 0) {
            g_server.sessions[i].last_access = time(NULL);
            const char *v = session_get_var(&g_server.sessions[i], key);
            if (v) {
...
static int session_set_var_safe(const char *sid, const char *key,
                                const char *val) {
    if (!sid || !sid[0] || !key || !val) return 0;
    pthread_mutex_lock(&g_server.session_mutex);
    for (int i = 0; i < g_server.session_count; i++) {
        if (g_server.sessions[i].active &&
            strcmp(g_server.sessions[i].id, sid) == 0) {
            g_server.sessions[i].last_access = time(NULL);
            int r = session_set_var(&g_server.sessions[i], key, val);
            pthread_mutex_unlock(&g_server.session_mutex);
            return r;
        }
    }
...
static int session_delete_var_safe(const char *sid, const char *key) {
    if (!sid || !sid[0] || !key) return 0;
    pthread_mutex_lock(&g_server.session_mutex);
    for (int i = 0; i < g_server.session_count; i++) {
        if (g_server.sessions[i].active &&
            strcmp(g_server.sessions[i].id, sid) == 0) {
            g_server.sessions[i].last_access = time(NULL);
            int r = session_delete_var(&g_server.sessions[i], key);
            pthread_mutex_unlock(&g_server.session_mutex);
            return r;
        }
    }
...
static int session_destroy_safe(const char *sid) {
...
static Route *find_route(HttpMethod method, const char *path,
    ...
    for (int pass = 0; pass < 2; pass++) {
        for (int i = 0; i < g_server.route_count; i++) {
            ...
            /* v5.2: HEAD は GET ルートにもマッチ */
            int method_match = (r->method == method) ||
                               (r->method == METHOD_ALL) ||
                               (method == METHOD_HEAD && r->method == METHOD_GET);
...
static void send_response_obj(socket_t fd, HttpResponse *resp) {
    ...
    /* v5.4: HTTP バージョンをリクエストに合わせる */
    HttpRequest *__req = ctx_req();
    const char *http_ver = (__req && strcmp(__req->http_version, "HTTP/1.0") == 0)
                            ? "HTTP/1.0" : "HTTP/1.1";
    /* v5.4: Connection ヘッダーを HTTP バージョンに対応 */
    const char *conn_val = "close";
    if (g_server.keep_alive_enabled) {
        if (__req && strcmp(__req->http_version, "HTTP/1.0") == 0) {
            const char *ch = __req ? get_header(__req, "Connection") : NULL;
            conn_val = (ch && strcasecmp(ch, "keep-alive") == 0)
                       ? "keep-alive" : "close";
        } else {
            conn_val = "keep-alive";
        }
    ...
    int hlen = snprintf(header, hdr_need,
        ...
        "Content-Length: %d\r\n"
        "Date: %s\r\n"
        "Server: hajimu_web/5.4\r\n"
        "Connection: %s\r\n",
        http_ver, resp->status_code, status_text(resp->status_code),
    ...
    int is_head = (__req && __req->method == METHOD_HEAD);
...
static void send_error(socket_t fd, int status_code, const char *message) {
    ...
    if (g_server.error_callback_count > 0 && hajimu_runtime_available()) {
        ...
        HttpRequest *__err_req = ctx_req();
        args[2] = __err_req
                  ? request_to_value(__err_req)
                  : hajimu_null();
 ...
 */
/* v5.4: テンプレート再帰深度制限 */
#define HW_TEMPLATE_MAX_DEPTH 16

static int template_render_inner(const char *tmpl, const TemplateVar *vars,
                           int var_count, char *output, int output_size,
                           int depth) {
    if (depth > HW_TEMPLATE_MAX_DEPTH) return 0;
    int oi = 0, ti = 0;
            ...
            if (tp[0] == '-' && tp[1] == '-') {
                ...
                if (is_moshi) {
                    ...
                    if (truthy) {
                        ...
                        block[block_len] = '\0';
                        oi += template_render_inner(block, vars, var_count,
                    ...
                    if (arr_val) {
                        ...
                        while (item) {
                            ...
                            oi += template_render_inner(block, loop_vars, lc,
                                output + oi, output_size - oi,
                                depth + 1);
...
/* ラッパー: 外部呼び出し用 (depth=0 から開始) */
static int template_render(const char *tmpl, const TemplateVar *vars,
                           int var_count, char *output, int output_size) {
    return template_render_inner(tmpl, vars, var_count, output, output_size, 0);
}
...
static char *render_template_file(const char *filename,
    ...
    /* v5.4: fread 戻り値チェック */
    size_t nread = fread(tmpl, 1, fsize, f);
    if ((long)nread != fsize) { free(tmpl); fclose(f); return NULL; }
    tmpl[fsize] = '\0';
    fclose(f);
...
static int try_static_directory(socket_t fd, const HttpRequest *req,
    ...
    if (stat(filepath, &st) == 0 && S_ISDIR(st.st_mode)) {
        ...
        if (stat(idx_path, &st) == 0) {
            ...
            closedir(d);
            hi += snprintf(html + hi, sizeof(html) - hi,
                "</ul><hr><p>hajimu_web/5.4</p></body></html>");
...
static int middleware_logger_before(HttpRequest *req, HttpResponse *resp) {
    ...
    if (strcmp(g_log_format, "combined") == 0) {
        ...
        const char *ref = get_header(req, "Referer");
        printf("%s - - [%s] \"%s %s HTTP/1.1\" - - \"%s\" \"%s\"\n",
               req->client_ip, time_str,
               method_to_string(req->method), req->path,
               ref ? ref : "-", ua ? ua : "-");
    } else if (strcmp(g_log_format, "dev") == 0) {
        printf("%s %s\n", method_to_string(req->method), req->path);
    } else if (strcmp(g_log_format, "short") == 0) {
        printf("%s %s %s\n", req->client_ip,
               method_to_string(req->method), req->path);
    } else if (strcmp(g_log_format, "tiny") == 0) {
        printf("%s %s\n", method_to_string(req->method), req->path);
    } else {
        printf("[%s] %s %s %s\n", time_str,
...
static int middleware_security_before(HttpRequest *req, HttpResponse *resp) {
    ...
    response_set_header(resp, "Content-Security-Policy",
                        "default-src 'self'");
    response_set_header(resp, "Strict-Transport-Security",
                        "max-age=31536000; includeSubDomains");
    response_set_header(resp, "Permissions-Policy",
...
static int middleware_rate_limit_before(HttpRequest *req, HttpResponse *resp) {
    ...
    pthread_mutex_lock(&g_server.rate_mutex);
    time_t now = time(NULL);
    int found = -1;
    ...
    if (found < 0) {
        if (g_server.rate_table_count < 256) {
            ...
            /* v5.4: LRU — 最古ウィンドウのエントリを追い出す */
            int oldest = 0;
            for (int i = 1; i < g_server.rate_table_count; i++) {
                if (g_server.rate_table[i].window_start <
                    g_server.rate_table[oldest].window_start)
                    oldest = i;
            }
            found = oldest;
            snprintf(g_server.rate_table[found].ip,
    ...
    if (g_server.rate_table[found].count > g_server.rate_limit_max) {
        ...
        pthread_mutex_unlock(&g_server.rate_mutex);
        return 1; /* 中断 → レスポンス送信 */
    }
    /* X-RateLimit ヘッダー */
    char rl_buf[32];
...
static void handle_client(socket_t client_fd, struct sockaddr_in *addr) {
  ...
  ka_loop: ;
    /* v5.4: リクエスト開始時刻を記録 (スレッドローカル) */
    clock_gettime(CLOCK_MONOTONIC, &ctx.request_start);
    char client_ip[64] = {0};
    ...
    /* 受信タイムアウト (Keep-Alive設定を反映) */
    struct timeval tv;
    tv.tv_sec = g_server.keep_alive_timeout > 0
              ? g_server.keep_alive_timeout : 5;
    tv.tv_usec = 0;
    setsockopt(client_fd, SOL_SOCKET, SO_RCVTIMEO,
               (const char *)&tv, sizeof(tv));

    /* v5.4: Slowloris 防御 — リクエスト受信のハードタイムアウト (30秒) */
    struct timespec recv_deadline;
    clock_gettime(CLOCK_MONOTONIC, &recv_deadline);
    recv_deadline.tv_sec += 30;

    while (1) {
        /* v5.4: ハードタイムアウト判定 */
        struct timespec now_ts;
        clock_gettime(CLOCK_MONOTONIC, &now_ts);
        if (now_ts.tv_sec > recv_deadline.tv_sec ||
            (now_ts.tv_sec == recv_deadline.tv_sec &&
             now_ts.tv_nsec >= recv_deadline.tv_nsec)) {
    ...
    /* v5.2: Expect: 100-continue 対応 */
    {
        const char *expect = get_header(&req, "Expect");
        if (expect && strcasecmp(expect, "100-continue") == 0) {
            const char *cont = "HTTP/1.1 100 Continue\r\n\r\n";
            send(client_fd, cont, (int)strlen(cont), 0);
        }
    }
    ...
    /* v5.4: Transfer-Encoding: chunked リクエストボディデコード */
    {
        const char *te = get_header(&req, "Transfer-Encoding");
        if (te && strcasestr(te, "chunked")) {
            /* ヘッダー末尾を見つける */
            char *hdr_end = strstr(raw, "\r\n\r\n");
            if (hdr_end) {
                char *chunk_start = hdr_end + 4;
                int remaining = total - (int)(chunk_start - raw);
                char *decoded = (char *)malloc(remaining + 1);
                int decoded_len = 0;
                if (decoded) {
                    char *p = chunk_start;
                    char *end = chunk_start + remaining;
                    while (p < end) {
                        /* チャンクサイズ読み取り */
                        char *crlf = strstr(p, "\r\n");
                        if (!crlf || crlf >= end) break;
                        long chunk_sz = strtol(p, NULL, 16);
                        if (chunk_sz <= 0) break; /* 終端チャンク */
                        p = crlf + 2;
                        if (p + chunk_sz > end) break;
                        memcpy(decoded + decoded_len, p, chunk_sz);
                        decoded_len += (int)chunk_sz;
                        p += chunk_sz;
    ...
    if (g_server.session_enabled) {
        ...
        if (sess) {
            ...
            snprintf(ctx.session_id, HW_SESSION_ID_LEN,
                     "%s", sess->id);
        }
    }
    ...
    /* v5.4: スレッドローカルコンテキストに req/resp を設定 */
    ctx.resp = &resp;
    ctx.req  = &req;
    ...
    if (route) {
        ...
        if (route->has_callback && route->is_wildcard == 2) {
            ...
            if (resp.sent) {
                ...
                if (g_server.session_enabled && req.session_id[0]) {
                    ...
                    snprintf(sc, sizeof(sc),
                        "hajimu_sid=%s; Path=/; HttpOnly; SameSite=Lax; Max-Age=%d%s",
                        req.session_id, stimeout,
                        g_server.tls_enabled ? "; Secure" : "");
            ...
            /* v5.1: 静的リダイレクトの Location ヘッダー */
            if (route->redirect_location[0])
                response_set_header(&resp, "Location",
    ...
    } else if ((req.method == METHOD_GET || req.method == METHOD_HEAD) &&
               serve_static_file(client_fd, &req)) {
        /* served by static file handler */
    } else {
        /* 405 vs 404 判定 (v5.2: Allow ヘッダー付き) */
        int path_exists = 0;
        char allow_methods[256] = {0};
        int allow_len = 0;
        for (int i = 0; i < g_server.route_count; i++) {
            ...
            if (match_route(&g_server.routes[i], req.path, dp, &dc)) {
                path_exists = 1;
                const char *m = method_to_string(g_server.routes[i].method);
                if (allow_len > 0)
                    allow_len += snprintf(allow_methods + allow_len,
                        sizeof(allow_methods) - allow_len, ", ");
                allow_len += snprintf(allow_methods + allow_len,
                    sizeof(allow_methods) - allow_len, "%s", m);
            }
        }
        if (path_exists) {
            /* v5.4: 405 もカスタムエラーハンドラ経由 + Allow ヘッダー付与 */
            HttpResponse err_resp;
            response_init(&err_resp);
            err_resp.status_code = 405;
            response_set_header(&err_resp, "Allow", allow_methods);
            snprintf(err_resp.content_type, sizeof(err_resp.content_type),
                     "text/html; charset=utf-8");
            /* エラーコールバックチェーン試行 */
            int handled = 0;
            if (g_server.error_callback_count > 0 && hajimu_runtime_available()) {
                Value args[3];
                args[0] = hajimu_number(405);
                args[1] = hajimu_string("Method Not Allowed");
                args[2] = request_to_value(&req);
                for (int ei = 0; ei < g_server.error_callback_count; ei++) {
                    pthread_mutex_lock(&g_server.callback_mutex);
                    Value result = hajimu_call(&g_server.error_callbacks[ei], 3, args);
                    pthread_mutex_unlock(&g_server.callback_mutex);
                    if (result.type == VALUE_STRING) {
                        response_set_body(&err_resp, result.string.data,
                                          result.string.length);
                        handled = 1;
                        break;
                    }
            ...
            if (!handled) {
                char err_body[512];
                int elen = snprintf(err_body, sizeof(err_body),
                    "405 Method Not Allowed (Allowed: %s)", allow_methods);
                response_set_body(&err_resp, err_body, elen);
            }
            send_response_obj(client_fd, &err_resp);
            if (err_resp.body) free(err_resp.body);
        } else {
    ...
    /* v5.4: Keep-Alive ループ判定 (HTTP/1.0 対応) */
    if (g_server.keep_alive_enabled && !g_shutdown) {
        ...
        if (strcmp(req.http_version, "HTTP/1.0") == 0) {
            /* HTTP/1.0: Connection: keep-alive が明示された場合のみ継続 */
            keep_going = (conn_hdr && strcasecmp(conn_hdr, "keep-alive") == 0);
        } else {
            /* HTTP/1.1: Connection: close でなければ継続 */
            keep_going = (!conn_hdr || strcasecmp(conn_hdr, "close") != 0);
        }
        if (ka_request_count < ka_max && keep_going) {
            goto ka_loop;
        }
    }
    close_socket(client_fd);
...
static Value fn_server_create(int argc, Value *argv) {
    ...
    if (g_server.running) {
        fprintf(stderr,
            "[hajimu_web] error: server already running\n");
        return hajimu_bool(false);
    }
    ...
    /* v5.1: セッション・レートリミッタ mutex */
    pthread_mutex_init(&g_server.session_mutex, NULL);
    pthread_mutex_init(&g_server.rate_mutex, NULL);
    /* v5.4: スレッドローカルコンテキスト初期化 */
    if (!g_ctx_key_initialized) {
        pthread_key_create(&g_ctx_key, NULL);
        g_ctx_key_initialized = 1;
    }
    g_shutdown = 0;
...
static Value fn_redirect(int argc, Value *argv) {
    ...
    /* v5.1: 静的リダイレクトにもLocationヘッダーを付与 */
    snprintf(r->redirect_location, sizeof(r->redirect_location),
             "%s", argv[1].string.data);
    return hajimu_bool(true);
}
    ...
    switch (val->type) {
        ...
        case VALUE_ARRAY: {
            ...
            if (is_kv) {
                ...
                for (int i = 0; i < val->array.length && written < buf_size - 4; i++) {
                    if (i > 0 && written < buf_size - 3) buf[written++] = ',';
                    Value *pair = &val->array.elements[i];
                    written += value_to_json(&pair->array.elements[0],
                                             buf+written, buf_size-written);
                    if (written < buf_size - 2) buf[written++] = ':';
        ...
        case VALUE_DICT: {
            if (written < buf_size - 1) buf[written++] = '{';
            for (int i = 0; i < val->dict.length && written < buf_size - 4; i++) {
                if (i > 0 && written < buf_size - 3) buf[written++] = ',';
                /* キー */
...
static Value fn_server_start(int argc, Value *argv) {
    ...
    pthread_mutex_destroy(&g_server.session_mutex);
    pthread_mutex_destroy(&g_server.rate_mutex);
...
static Value fn_send_file(int argc, Value *argv) {
    ...
    /* v5.4: fread 戻り値チェック */
    if ((long)fread(data, 1, fsize, f) != fsize) { free(data); fclose(f); return hajimu_bool(false); }
    fclose(f);
...
/* セッション取得(キー) — v5.4: スレッドセーフ */
static Value fn_session_get(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_STRING) return hajimu_null();
    char buf[HW_MAX_HEADER_VALUE];
    if (session_get_var_safe(ctx_session_id(), argv[0].string.data,
                             buf, sizeof(buf)))
        return hajimu_string(buf);
    return hajimu_null();
}

/* セッション設定(キー, 値) — v5.4: スレッドセーフ */
static Value fn_session_set(int argc, Value *argv) {
    if (argc < 2 || argv[0].type != VALUE_STRING ||
        argv[1].type != VALUE_STRING) return hajimu_bool(false);
    return hajimu_bool(session_set_var_safe(ctx_session_id(),
                       argv[0].string.data, argv[1].string.data));
}

/* セッション削除(キー) — v5.4: スレッドセーフ */
static Value fn_session_del(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_STRING) return hajimu_bool(false);
    return hajimu_bool(session_delete_var_safe(ctx_session_id(),
                       argv[0].string.data));
}

/* セッション破棄() — v5.4: スレッドセーフ */
static Value fn_session_destroy(int argc, Value *argv) {
    (void)argc; (void)argv;
    int ok = session_destroy_safe(ctx_session_id());
    if (ok) {
        char *__sid = ctx_session_id();
        if (__sid) __sid[0] = '\0';
    }
...
static Value fn_sse_route(int argc, Value *argv) {
    ...
    /* SSE mutex は fn_server_create で初期化済み — 二重初期化しない */
    return hajimu_bool(true);
}
...
static Value fn_upload_save(int argc, Value *argv) {
    ...
    /* パストラバーサル防御: ファイル名から危険な文字を除去 */
    char safe_filename[256];
    snprintf(safe_filename, sizeof(safe_filename), "%s", uf->filename);
    if (strstr(safe_filename, "..") || safe_filename[0] == '/' ||
        safe_filename[0] == '\\') {
        /* basename のみ抽出 */
        const char *bn = strrchr(safe_filename, '/');
        if (!bn) bn = strrchr(safe_filename, '\\');
        if (bn) memmove(safe_filename, bn + 1, strlen(bn + 1) + 1);
        /* まだ .. を含む場合は拒否 */
        if (strstr(safe_filename, "..") || safe_filename[0] == '\0')
            return hajimu_bool(false);
    }
    char path[HW_MAX_PATH];
    if (argc >= 2 && argv[1].type == VALUE_STRING) {
        ...
        /* 明示パスでもトラバーサルチェック */
        if (strstr(path, "..")) return hajimu_bool(false);
    } else if (g_server.upload_dir[0])
        snprintf(path, sizeof(path), "%s/%s",
                 g_server.upload_dir, safe_filename);
    else
        snprintf(path, sizeof(path), "%s", safe_filename);
...
static Value fn_write_chunk(int argc, Value *argv) {
    ...
    if (!resp->sent) {
        ...
        get_http_date(date, sizeof(date));
        /* v5.4: HTTP バージョンをリクエストに合わせる */
        HttpRequest *__creq = ctx_req();
        const char *__cver = (__creq && strcmp(__creq->http_version, "HTTP/1.0") == 0)
                              ? "HTTP/1.0" : "HTTP/1.1";
        char hdr[2048];
        int hlen = snprintf(hdr, sizeof(hdr),
            ...
            "Server: hajimu_web/5.4\r\n"
            "Connection: %s\r\n",
            __cver, resp->status_code, status_text(resp->status_code),
...
static Value fn_end_chunk(int argc, Value *argv) {
    ...
    send(fd, "0\r\n\r\n", 5, 0);
    resp->sent = 1;
    resp->chunked_started = 0;
    return hajimu_bool(true);
}

/* --- コンテンツネゴシエーション --- */

/* v5.1: Accept ヘッダーをq値付きで解析 */
typedef struct {
    char type[128];
    double q;
} AcceptEntry;

static int parse_accept_header(const char *accept, AcceptEntry *entries, int max) {
    int count = 0;
    char buf[1024];
    ...
    while (token && count < max) {
        ...
        if (semi) {
            *semi = '\0';
            char *qp = strstr(semi + 1, "q=");
            if (qp) entries[count].q = strtod(qp + 2, NULL);
        }
        /* 末尾空白除去 */
        char *end = token + strlen(token) - 1;
        while (end > token && *end == ' ') *end-- = '\0';
        snprintf(entries[count].type, sizeof(entries[count].type), "%s", token);
        count++;
...
static int accept_matches(const char *accept_type, const char *check_type) {
    if (strcmp(accept_type, "*/*") == 0) return 1;
    if (strcmp(accept_type, check_type) == 0) return 1;
    /* e.g. text/star vs text/html */
    char major[64];
    snprintf(major, sizeof(major), "%s", check_type);
    char *slash = strchr(major, '/');
    if (slash) {
        snprintf(slash, sizeof(major) - (slash - major), "/*");
        if (strcmp(accept_type, major) == 0) return 1;
    }
    return 0;
}
...
static Value fn_accepts(int argc, Value *argv) {
    ...
    AcceptEntry entries[32];
    int n = parse_accept_header(accept, entries, 32);
    for (int i = 0; i < n; i++) {
        if (entries[i].q > 0 && accept_matches(entries[i].type, type))
            return hajimu_bool(true);
    }
    return hajimu_bool(false);
}
...
static Value fn_format(int argc, Value *argv) {
    ...
    /* q値降順で、最初にマッチするハンドラを実行 */
    for (int ai = 0; ai < n; ai++) {
        if (entries[ai].q <= 0) continue;
        for (int i = 0; i < argv[0].array.length; i++) {
            ...
            if (accept_matches(entries[ai].type, type)) {
...
static Value fn_protocol(int argc, Value *argv) {
    ...
    HttpRequest *__pr = ctx_req();
    if (g_server.trust_proxy && __pr) {
        const char *fp = get_header(__pr, "X-Forwarded-Proto");
...
/* IP一覧取得() — X-Forwarded-For の全プロキシチェーンを配列で返す */
static Value fn_ips(int argc, Value *argv) {
    (void)argc; (void)argv;
    HttpRequest *req = ctx_req();
    if (!req) return hajimu_array();
    Value arr = hajimu_array();
    if (g_server.trust_proxy) {
        const char *xff = get_header(req, "X-Forwarded-For");
        if (xff) {
            char buf[1024];
            snprintf(buf, sizeof(buf), "%s", xff);
            char *saveptr = NULL;
            ...
            while (tok) {
                ...
                hajimu_array_push(&arr, hajimu_string(tok));
...
/* Vary設定(フィールド) — Vary ヘッダーを追加/追記 */
static Value fn_vary(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_STRING)
        return hajimu_bool(false);
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_bool(false);
    /* 既存の Vary ヘッダーを検索 */
    for (int i = 0; i < resp->header_count; i++) {
        if (strcasecmp(resp->headers[i].key, "Vary") == 0) {
            /* 既に含まれていなければ追記 */
            if (!strstr(resp->headers[i].value, argv[0].string.data)) {
                char combined[HW_MAX_HEADER_VALUE];
                snprintf(combined, sizeof(combined), "%s, %s",
                         resp->headers[i].value, argv[0].string.data);
                snprintf(resp->headers[i].value, sizeof(resp->headers[i].value),
                         "%s", combined);
            }
            return hajimu_bool(true);
        }
    }
    response_set_header(resp, "Vary", argv[0].string.data);
    return hajimu_bool(true);
}

/* リンク設定(配列) — Link ヘッダーを設定
   配列 = [["next", "/page/2"], ["last", "/page/5"]] */
static Value fn_links(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_ARRAY)
        return hajimu_bool(false);
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_bool(false);
    char link_buf[2048] = {0};
    int written = 0;
    for (int i = 0; i < argv[0].array.length; i++) {
        Value *pair = &argv[0].array.elements[i];
        if (pair->type != VALUE_ARRAY || pair->array.length < 2) continue;
        if (pair->array.elements[0].type != VALUE_STRING ||
            pair->array.elements[1].type != VALUE_STRING) continue;
        if (written > 0)
            written += snprintf(link_buf + written,
                sizeof(link_buf) - written, ", ");
        written += snprintf(link_buf + written, sizeof(link_buf) - written,
            "<%s>; rel=\"%s\"",
            pair->array.elements[1].string.data,
            pair->array.elements[0].string.data);
    }
    if (written > 0)
        response_set_header(resp, "Link", link_buf);
    return hajimu_bool(true);
}

/* タイプ設定(短縮名) — Content-Type を短縮名で設定
   "json" → "application/json", "html" → "text/html", etc. */
static Value fn_type(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_STRING)
        return hajimu_bool(false);
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_bool(false);
    const char *t = argv[0].string.data;
    const char *full = t; /* デフォルト: そのまま */
    if (strcmp(t, "json") == 0) full = "application/json; charset=utf-8";
    else if (strcmp(t, "html") == 0) full = "text/html; charset=utf-8";
    else if (strcmp(t, "text") == 0) full = "text/plain; charset=utf-8";
    else if (strcmp(t, "xml") == 0) full = "application/xml; charset=utf-8";
    else if (strcmp(t, "css") == 0) full = "text/css; charset=utf-8";
    else if (strcmp(t, "js") == 0) full = "application/javascript; charset=utf-8";
    else if (strcmp(t, "png") == 0) full = "image/png";
    else if (strcmp(t, "jpg") == 0 || strcmp(t, "jpeg") == 0) full = "image/jpeg";
    else if (strcmp(t, "gif") == 0) full = "image/gif";
    else if (strcmp(t, "svg") == 0) full = "image/svg+xml";
    else if (strcmp(t, "pdf") == 0) full = "application/pdf";
    else if (strcmp(t, "bin") == 0) full = "application/octet-stream";
    else if (strcmp(t, "form") == 0) full = "application/x-www-form-urlencoded";
    else if (strcmp(t, "multipart") == 0) full = "multipart/form-data";
    snprintf(resp->content_type, sizeof(resp->content_type), "%s", full);
    return hajimu_bool(true);
}

/* ステータス送信(コード) — ステータスコードとその説明テキストを即送信 */
static Value fn_send_status(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_NUMBER)
        return hajimu_bool(false);
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_bool(false);
    ...
    const char *text = status_text(code);
    snprintf(resp->content_type, sizeof(resp->content_type),
             "text/plain; charset=utf-8");
    response_set_body(resp, text, (int)strlen(text));
    return hajimu_bool(true);
}

/* フレッシュ確認() — リクエストがキャッシュ的に "新鮮" か判定
   ETag / If-None-Match + If-Modified-Since 対応 */
/* v5.2: HTTP日付パース (RFC 7231) */
static time_t parse_http_date(const char *date_str) {
    struct tm tm;
    memset(&tm, 0, sizeof(tm));
    /* RFC 7231: "Sun, 06 Nov 1994 08:49:37 GMT" */
    if (strptime(date_str, "%a, %d %b %Y %H:%M:%S GMT", &tm))
        return timegm(&tm);
...
static Value fn_is_fresh(int argc, Value *argv) {
    (void)argc; (void)argv;
    HttpRequest *req = ctx_req();
    HttpResponse *resp = ctx_resp();
    if (!req || !resp) return hajimu_bool(false);
    /* If-None-Match vs ETag */
    const char *inm = get_header(req, "If-None-Match");
    if (inm) {
        for (int i = 0; i < resp->header_count; i++) {
            if (strcasecmp(resp->headers[i].key, "ETag") == 0)
                return hajimu_bool(strcmp(inm, resp->headers[i].value) == 0);
        }
    }
    /* v5.2: If-Modified-Since — 日付比較 */
    const char *ims = get_header(req, "If-Modified-Since");
    if (ims) {
        time_t ims_time = parse_http_date(ims);
        if (ims_time == (time_t)-1) return hajimu_bool(false);
        for (int i = 0; i < resp->header_count; i++) {
            if (strcasecmp(resp->headers[i].key, "Last-Modified") == 0) {
                time_t lm_time = parse_http_date(resp->headers[i].value);
                if (lm_time == (time_t)-1) return hajimu_bool(false);
                return hajimu_bool(lm_time <= ims_time);
            }
        }
    }
    return hajimu_bool(false);
...
static Value fn_is_stale(int argc, Value *argv) {
    Value fresh = fn_is_fresh(argc, argv);
    return hajimu_bool(!(fresh.type == VALUE_BOOL && fresh.boolean));
}
...
/* v5.2: VALUE_DICT からキーで値を取得するヘルパー */
static Value dict_lookup(Value *dict, const char *key) {
    if (dict->type != VALUE_DICT) return hajimu_null();
    for (int i = 0; i < dict->dict.length; i++) {
        if (dict->dict.keys[i] && strcmp(dict->dict.keys[i], key) == 0)
...
/* --- Cookie 構造化オプション --- */
/* Cookie詳細設定(名前, 値, オプション辞書) */
/* オプション辞書: {"maxAge": 3600, "httpOnly": 真, "secure": 真, "sameSite": "Lax", "path": "/", "domain": ".example.com"} */
static Value fn_set_cookie_opts(int argc, Value *argv) {
    if (argc < 3 || argv[0].type != VALUE_STRING ||
        argv[1].type != VALUE_STRING || argv[2].type != VALUE_DICT)
        return hajimu_bool(false);
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_bool(false);

    char cookie[HW_MAX_HEADER_VALUE];
    int clen = snprintf(cookie, sizeof(cookie), "%s=%s",
        argv[0].string.data, argv[1].string.data);

    /* パス (デフォルト: /) */
    Value vpath = dict_lookup(&argv[2], "path");
    if (vpath.type == VALUE_STRING)
        clen += snprintf(cookie + clen, sizeof(cookie) - clen,
            "; Path=%s", vpath.string.data);
    else
        clen += snprintf(cookie + clen, sizeof(cookie) - clen, "; Path=/");

    /* domain */
    Value vdom = dict_lookup(&argv[2], "domain");
    if (vdom.type == VALUE_STRING)
        clen += snprintf(cookie + clen, sizeof(cookie) - clen,
            "; Domain=%s", vdom.string.data);

    /* maxAge */
    Value vmax = dict_lookup(&argv[2], "maxAge");
    if (vmax.type == VALUE_NUMBER)
        clen += snprintf(cookie + clen, sizeof(cookie) - clen,
            "; Max-Age=%d", (int)vmax.number);

    /* httpOnly */
    Value vhttp = dict_lookup(&argv[2], "httpOnly");
    if (vhttp.type == VALUE_BOOL && vhttp.boolean)
        clen += snprintf(cookie + clen, sizeof(cookie) - clen, "; HttpOnly");

    /* secure */
    Value vsec = dict_lookup(&argv[2], "secure");
    if (vsec.type == VALUE_BOOL && vsec.boolean)
        clen += snprintf(cookie + clen, sizeof(cookie) - clen, "; Secure");

    /* sameSite */
    Value vss = dict_lookup(&argv[2], "sameSite");
    if (vss.type == VALUE_STRING)
        clen += snprintf(cookie + clen, sizeof(cookie) - clen,
            "; SameSite=%s", vss.string.data);
...
/* --- req.get(header) — リクエストヘッダー取得 --- */
static Value fn_req_get(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_STRING)
        return hajimu_null();
    HttpRequest *req = ctx_req();
    if (!req) return hajimu_null();
    const char *val = get_header(req, argv[0].string.data);
    return val ? hajimu_string(val) : hajimu_null();
}

/* --- req.is(type) — Content-Type 一致チェック --- */
static Value fn_req_is(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_STRING)
        return hajimu_bool(false);
    HttpRequest *req = ctx_req();
    if (!req) return hajimu_bool(false);
    const char *ct = get_header(req, "Content-Type");
    if (!ct) return hajimu_bool(false);
    const char *check = argv[0].string.data;
    /* 短縮形対応 */
    if (strcmp(check, "json") == 0) check = "application/json";
    else if (strcmp(check, "html") == 0) check = "text/html";
    else if (strcmp(check, "text") == 0) check = "text/plain";
    else if (strcmp(check, "xml") == 0) check = "application/xml";
    else if (strcmp(check, "form") == 0) check = "application/x-www-form-urlencoded";
    else if (strcmp(check, "multipart") == 0) check = "multipart/form-data";
    return hajimu_bool(strstr(ct, check) != NULL);
}

/* --- res.get(header) — レスポンスヘッダー読み取り --- */
static Value fn_res_get(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_STRING)
        return hajimu_null();
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_null();
    for (int i = 0; i < resp->header_count; i++) {
        if (strcasecmp(resp->headers[i].key, argv[0].string.data) == 0)
            return hajimu_string(resp->headers[i].value);
    }
    return hajimu_null();
}
...
/* --- res.locals 操作 --- */
/* ローカル変数設定(キー, 値) */
static Value fn_locals_set(int argc, Value *argv) {
    if (argc < 2 || argv[0].type != VALUE_STRING ||
        argv[1].type != VALUE_STRING)
        return hajimu_bool(false);
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_bool(false);
    /* 既存キー更新 */
    for (int i = 0; i < resp->local_count; i++) {
        if (strcmp(resp->locals[i].key, argv[0].string.data) == 0) {
            snprintf(resp->locals[i].value, HW_MAX_HEADER_VALUE,
                     "%s", argv[1].string.data);
            return hajimu_bool(true);
        }
    }
    /* 新規追加 */
    if (resp->local_count < HW_MAX_QUERY_PARAMS) {
        snprintf(resp->locals[resp->local_count].key, 256,
                 "%s", argv[0].string.data);
        snprintf(resp->locals[resp->local_count].value, HW_MAX_HEADER_VALUE,
                 "%s", argv[1].string.data);
        resp->local_count++;
        return hajimu_bool(true);
    }
...
/* ローカル変数取得(キー) */
static Value fn_locals_get(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_STRING)
        return hajimu_null();
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_null();
    for (int i = 0; i < resp->local_count; i++) {
        if (strcmp(resp->locals[i].key, argv[0].string.data) == 0)
            return hajimu_string(resp->locals[i].value);
    }
    return hajimu_null();
}

/* --- セッション再生成 (セッション固定攻撃対策) v5.4: スレッドセーフ --- */
static Value fn_session_regenerate(int argc, Value *argv) {
    (void)argc; (void)argv;
    if (!g_server.session_enabled) return hajimu_bool(false);
    char *old_sid = ctx_session_id();
    pthread_mutex_lock(&g_server.session_mutex);
    /* 古いセッションを検索 */
    Session *old = NULL;
    if (old_sid && old_sid[0]) {
        for (int i = 0; i < g_server.session_count; i++) {
            if (g_server.sessions[i].active &&
                strcmp(g_server.sessions[i].id, old_sid) == 0) {
                old = &g_server.sessions[i];
                break;
            }
        }
    }
    pthread_mutex_unlock(&g_server.session_mutex);
    Session *new_s = create_session();
    if (!new_s) return hajimu_bool(false);
    /* 古いセッションの変数を新セッションにコピー */
    pthread_mutex_lock(&g_server.session_mutex);
    if (old && old->active) {
        memcpy(new_s->vars, old->vars, sizeof(old->vars));
        new_s->var_count = old->var_count;
        old->active = 0;
    }
    pthread_mutex_unlock(&g_server.session_mutex);
    HttpRequest *req = ctx_req();
    if (req)
        snprintf(req->session_id, HW_SESSION_ID_LEN, "%s", new_s->id);
    if (old_sid) snprintf(old_sid, HW_SESSION_ID_LEN, "%s", new_s->id);
    return hajimu_bool(true);
}

/* --- ログフォーマット設定 --- */

static Value fn_log_format(int argc, Value *argv) {
    if (argc < 1 || argv[0].type != VALUE_STRING)
        return hajimu_bool(false);
    snprintf(g_log_format, sizeof(g_log_format), "%s", argv[0].string.data);
    return hajimu_bool(true);
}

/* --- X-Response-Time ミドルウェア --- */

static Value fn_response_time(int argc, Value *argv) {
    (void)argc; (void)argv;
    /* v5.4: スレッドローカルのリクエスト開始時刻から計算 */
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_null();
    RequestContext *__ctx = get_ctx();
    if (!__ctx) return hajimu_null();
    struct timespec now;
    clock_gettime(CLOCK_MONOTONIC, &now);
    double ms = (now.tv_sec - __ctx->request_start.tv_sec) * 1000.0
              + (now.tv_nsec - __ctx->request_start.tv_nsec) / 1e6;
    char timebuf[64];
    snprintf(timebuf, sizeof(timebuf), "%.3fms", ms);
    response_set_header(resp, "X-Response-Time", timebuf);
    return hajimu_string(timebuf);
}

/* --- X-Request-Id 生成 --- */
static Value fn_request_id(int argc, Value *argv) {
    (void)argc; (void)argv;
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_null();
    /* 既に X-Request-Id があればそれを返す */
    for (int i = 0; i < resp->header_count; i++) {
        if (strcasecmp(resp->headers[i].key, "X-Request-Id") == 0)
            return hajimu_string(resp->headers[i].value);
    }
    /* 新規生成 (簡易UUID風) */
    char id[37];
    ...
    uint32_t r[4];
    for (int i = 0; i < 4; i++) r[i] = arc4random();
    snprintf(id, sizeof(id),
        "%08x-%04x-%04x-%04x-%04x%08x",
        r[0], (r[1] >> 16) & 0xFFFF, (r[1] & 0x0FFF) | 0x4000,
        ((r[2] >> 16) & 0x3FFF) | 0x8000,
        r[2] & 0xFFFF, r[3]);
#else
    FILE *urand = fopen("/dev/urandom", "rb");
    unsigned char rb[16];
    if (urand) { fread(rb, 1, 16, urand); fclose(urand); }
    else memset(rb, 0, 16);
    rb[6] = (rb[6] & 0x0F) | 0x40; rb[8] = (rb[8] & 0x3F) | 0x80;
    int p = 0;
    for (int i = 0; i < 16; i++) {
        id[p++] = hex[(rb[i] >> 4) & 0xF];
        id[p++] = hex[rb[i] & 0xF];
        if (i == 3 || i == 5 || i == 7 || i == 9) id[p++] = '-';
    }
    id[p] = '\0';
#endif
    (void)hex;
    response_set_header(resp, "X-Request-Id", id);
    return hajimu_string(id);
}

/* --- req.xhr — XMLHttpRequest 判定 --- */
static Value fn_is_xhr(int argc, Value *argv) {
    (void)argc; (void)argv;
    HttpRequest *req = ctx_req();
    if (!req) return hajimu_bool(false);
    const char *xrw = get_header(req, "X-Requested-With");
    return hajimu_bool(xrw && strcasecmp(xrw, "XMLHttpRequest") == 0);
}

/* --- req.subdomains — サブドメイン配列 --- */
static Value fn_subdomains(int argc, Value *argv) {
    (void)argc; (void)argv;
    HttpRequest *req = ctx_req();
    if (!req) return hajimu_null();
    const char *host = get_header(req, "Host");
    if (!host) return hajimu_null();
    char hostbuf[256];
    ...
    for (int i = pc - 3; i >= 0; i--)
        hajimu_array_push(&arr, hajimu_string(parts[i]));
    return arr;
}

/* --- req.originalUrl — クエリ付きオリジナルURL --- */
static Value fn_original_url(int argc, Value *argv) {
    (void)argc; (void)argv;
    HttpRequest *req = ctx_req();
    if (!req) return hajimu_null();
    if (req->raw_query[0]) {
        char url[HW_MAX_PATH * 2];
        snprintf(url, sizeof(url), "%s?%s", req->path, req->raw_query);
        return hajimu_string(url);
    }
    return hajimu_string(req->path);
}

/* --- res.append(header, value) — ヘッダー追記 --- */
static Value fn_res_append(int argc, Value *argv) {
    if (argc < 2 || argv[0].type != VALUE_STRING ||
        argv[1].type != VALUE_STRING)
        return hajimu_bool(false);
    HttpResponse *resp = ctx_resp();
    ...
    for (int i = 0; i < resp->header_count; i++) {
        if (strcasecmp(resp->headers[i].key, argv[0].string.data) == 0) {
            ...
            snprintf(combined, sizeof(combined), "%s, %s",
                resp->headers[i].value, argv[1].string.data);
            snprintf(resp->headers[i].value, HW_MAX_HEADER_VALUE,
                "%s", combined);
            return hajimu_bool(true);
        }
    }
...
/* --- res.headersSent — ヘッダー送信済みか確認 --- */
static Value fn_headers_sent(int argc, Value *argv) {
    (void)argc; (void)argv;
    HttpResponse *resp = ctx_resp();
    if (!resp) return hajimu_bool(false);
    return hajimu_bool(resp->sent);
}

/* --- サーバー情報 v5.2 (辞書型で返す) --- */
static Value fn_server_info_v2(int argc, Value *argv) {
    (void)argc; (void)argv;
    /* VALUE_DICT を手動構築 */
    int nkeys = 10;
    Value info;
    memset(&info, 0, sizeof(info));
    info.type = VALUE_DICT;
    info.dict.keys = (char **)calloc(nkeys, sizeof(char *));
    info.dict.values = (Value *)calloc(nkeys, sizeof(Value));
    info.dict.length = nkeys;
    info.dict.capacity = nkeys;
    info.dict.keys[0] = strdup("version");
    info.dict.values[0] = hajimu_string("5.2");
    info.dict.keys[1] = strdup("port");
    info.dict.values[1] = hajimu_number(g_server.port);
    info.dict.keys[2] = strdup("routes");
    info.dict.values[2] = hajimu_number(g_server.route_count);
    info.dict.keys[3] = strdup("middlewares");
    info.dict.values[3] = hajimu_number(g_server.middleware_count);
    info.dict.keys[4] = strdup("total_requests");
    info.dict.values[4] = hajimu_number((double)g_server.total_requests);
    info.dict.keys[5] = strdup("errors");
    info.dict.values[5] = hajimu_number((double)g_server.error_count);
    info.dict.keys[6] = strdup("active_threads");
    info.dict.values[6] = hajimu_number(g_server.active_threads);
    info.dict.keys[7] = strdup("keep_alive");
    info.dict.values[7] = hajimu_bool(g_server.keep_alive_enabled);
    info.dict.keys[8] = strdup("trust_proxy");
    info.dict.values[8] = hajimu_bool(g_server.trust_proxy);
    info.dict.keys[9] = strdup("tls");
    info.dict.values[9] = hajimu_bool(g_server.tls_enabled);
    return info;
}
...
static HajimuPluginFunc functions[] = {
    ...
    {"リンク設定",          fn_links,                1, 1},
    {"タイプ設定",          fn_type,                 1, 1},
    {"ステータス送信",      fn_send_status,          1, 1},
    {"フレッシュ確認",      fn_is_fresh,             0, 0},
    {"陳腐確認",            fn_is_stale,             0, 0},
    /* v5.2 新規API (16) */
    {"Cookie詳細設定",      fn_set_cookie_opts,      3, 3},
    {"リクエストヘッダー取得", fn_req_get,             1, 1},
    {"リクエストタイプ確認",   fn_req_is,             1, 1},
    {"レスポンスヘッダー取得", fn_res_get,             1, 1},
    {"ローカル変数設定",      fn_locals_set,          2, 2},
    {"ローカル変数取得",      fn_locals_get,          1, 1},
    {"セッション再生成",      fn_session_regenerate,  0, 0},
    {"ログフォーマット",      fn_log_format,          1, 1},
    {"応答時間",             fn_response_time,        0, 0},
    {"リクエストID",         fn_request_id,           0, 0},
    {"XHR確認",             fn_is_xhr,               0, 0},
    {"サブドメイン",         fn_subdomains,           0, 0},
    {"オリジナルURL",        fn_original_url,         0, 0},
    {"ヘッダー追記",         fn_res_append,           2, 2},
    {"ヘッダー送信済み",     fn_headers_sent,         0, 0},
};
...
HAJIMU_PLUGIN_EXPORT HajimuPluginInfo *hajimu_plugin_init(void) {
    static HajimuPluginInfo info = {
        ...
        .version        = "5.4.0",
        .author         = "はじむ開発チーム",
        .description    = "HTTP ウェブサーバー v5.4 — スレッドローカルコンテキスト・TOCTOU防止・HTTP/1.0対応・Slowloris防御・レートLRU・テンプレート再帰制限・chunkedリクエスト対応",
        .functions      = functions,
